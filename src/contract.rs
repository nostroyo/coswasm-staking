use cosmwasm_std::{
    coins, entry_point, to_binary, Addr, BankMsg, Binary, Coin, Decimal, Deps, DepsMut, Env,
...
use crate::state::{State, AMOUNT_BY_USER, GAIN_BY_USER, STATE};
...
pub fn execute_withdraw(
    ...
    if amount > amount_for_user || amount == Uint128::zero() {
        return Err(ContractError::WrongArgument {
    ...
    GAIN_BY_USER.update(
        deps.storage,
        &info.sender,
        |gain_before_withdraw: Option<Uint128>| -> StdResult<_> {
            Ok(gain_before_withdraw
                .unwrap_or_default()
                .checked_sub(gain_to_withdraw)?)
        },
    )?;

    AMOUNT_BY_USER.update(
        deps.storage,
        &info.sender,
        |amount_before_withdraw: Option<Uint128>| -> StdResult<_> {
            Ok(amount_before_withdraw
                .unwrap_or_default()
                .checked_sub(amount)?)
        },
    )?;
...
pub fn execute_update_pool_total(
    deps: DepsMut,
    info: MessageInfo,
) -> Result<Response, ContractError> {
    ...
    for user_amount in all_user {
        println!(
            "user {} amount {}",
            std::str::from_utf8(&user_amount.0).unwrap(),
            user_amount.1
        );
        current_amount = user_amount.1;
        current_user_address =
            Addr::unchecked(std::str::from_utf8(&user_amount.0).unwrap().to_string());
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetPoolTotalAmount {} => {
            let state = STATE.load(deps.storage)?;
            to_binary(&PoolTotalAmountResponse {
                amount: state.pool_total_amount,
            })
        }
...
mod tests {
    ...
    fn withdraw_all_amount() {
        ...
        match _res {
            Err(ContractError::WrongArgument { name }) => {
                assert_eq!(name, "amount".to_string())
            }
            _ => panic!("Must return WrongDeposit error"),
        }
        ...
        let info = mock_info("user1", &[]);
        let msg = ExecuteMsg::Withdraw {
            amount: Uint128::from(2_000_001u128),
        };
        let res = execute(deps.as_mut(), mock_env(), info, msg);
        match res {
            Err(ContractError::WrongArgument { name }) => {
